...
if (isset($_POST['submission'])) {
    ...
    $recordnumber = sanitize_text_field($_POST['recordnumber']);
    ...
    $table_query = "CREATE TABLE IF NOT EXISTS {$wpdb->prefix}companyrecord (
    ...
    $result = $wpdb->insert($wpdb->prefix . 'companyrecord', $data);

    if ($result === false) {
        $wpdb_error = $wpdb->last_error;
        echo "<script> console.log('Database Error: $wpdb_error'); </script>";
    } else {
        echo "<script> alert('Data Saved'); </script>";

        // generate_pdf($data);
		header("Location: /form-submission-data/");  // Replace "/specific-url" with your desired URL
    exit();
    }
}
...
if (isset($_POST['update_submission'])) {
    ...
    $id= sanitize_text_field($_POST['record_id']);
    ...
    $result = $wpdb->update($wpdb->prefix . 'companyrecord', $data, array('id' => $id));
    ...
    if ($result === false) {
        ...
        if ($result === 0) {
            echo "<script> console.log('No rows updated. Check if the record exists with ID: $id'); </script>";
        } else {
            echo "<script> console.log('Data Updated successfully'); </script>";
        }

        // Redirect to the employee list page after successful update
        wp_redirect(home_url('/form-submission-data'));
        exit();
    }
}
...
//     $mpdf->Output('employee_information.pdf', 'F');
// }


// Home redirect & login handler
